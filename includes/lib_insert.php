 ...
 * @return  string
 */
function insert_show_div_info($goods_number,$goods_id,$goods_amount,$real_goods_count)
{
...
function insert_get_ad($arr)
{
	$arr['cat_name'];
	$arr['type'];
	if(!empty($arr['top_name']))
	{
		$ad_name = $arr['top_name'].'_'.$arr['cat_name'].$arr['type'];
	}
	else
	{
    ...
    if (!empty($arr['cat_name']) && !empty($arr['type']))
    {
        $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.link_man,a.ad_code, a.ad_name, p.ad_width, ' .
	...
	$row["ad_link"] = urlencode($row["ad_link"]);
	...
	if($arr['title'] == 'show')
	{
		$GLOBALS['smarty']->assign('title', 'show');
	}
...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
			...
			'<li>
						<a href="'.$goods['url'].'" target="_blank"><img src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'" width="174" height="174" /></a>
						<div style="padding:5px 10px;text-align:left;">
							<a href="'.$goods['url'].'" target="_blank" title="'.$goods['goods_name'].'">'.$goods['short_name'].'</a>
						</div>
						<div class="price">
							<strong class="red">  优惠价： </strong>
							<strong class="red">' . '￥' .$goods['shop_price'].'</strong>
							<s style="padding-left:10px;color:#898989">' . '￥' .$goods['market_price'].'</s>
						</div>
				   </li>';
        }
...
function insert_yhd_history()
{
    ...
    $output = $GLOBALS['smarty']->fetch('library/yhd_history_list.lbi');
    $GLOBALS['smarty']->caching = $need_cache;
...
function insert_history_flow()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
			...
			<li>
			<h2>
				<a target="_blank" href="'.$goods['url'].'" title="'.$goods['goods_name'].'">
				<img src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'">
				</a>
			</h2>
			<dl>
				<dt><a target="_blank" href="'.$goods['url'].'">'.$goods['goods_name'].'</a><br></dt>
				<dd>本店价: <b>'.$goods['shop_price'].'</b> 元<br><br>
			</dd>
			</dl></li>';
        }
...
function insert_cart_info()
{
	...
	$str = '<a rel="nofollow" class="btn-shopcart" href="flow.php">购物车</a><span class="txt-cartcount" style="display: inline;">'.$number.'</span><div class="shopcart-list"><ul>';
	
	$sql = 'SELECT c.rec_id,c.goods_name,c.goods_id,c.goods_price,c.goods_number,g.goods_thumb FROM '.$GLOBALS['ecs']->table('cart'). " AS c LEFT JOIN ".$GLOBALS['ecs']->table('goods')." AS g ON g.goods_id=c.goods_id WHERE session_id = '" . SESS_ID . "' AND rec_type = '" . CART_GENERAL_GOODS . "'";
	$res = $GLOBALS['db']->GetAll($sql);
	foreach($res as $row)
	{
		$str .= ' <li>
		  <a class="prod-info" target="_blank" href="goods.php?id='.$row['goods_id'].'">
		  <img width="60" height="98" alt="'.$row['goods_name'].'" src="'.$row['goods_thumb'].'">
		  <span class="name">'.$row['goods_name'].'</span>
		  <span class="price"><strong>¥ '.$row['goods_price'].'</strong> × <em>'.$row['goods_number'].'</em></span></a>
		  <a title="'.$row['goods_name'].'" class="btn-remove" href="javascript:;" onclick="yesmywine_drop_goods('.$row['rec_id'].')">删除</a>
    ...
    $str .= '</ul><div class="shopcart-sum"><a class="btn-viewcart" href="flow.php">查看购物车</a>
			<p>共<em>'.$number.'</em>件商品<br>
            总计： ¥<strong>'.$amount.'</strong></p>
      	 </div></div>';
	}
	else
	{
		$str = '<a rel="nofollow" class="btn-shopcart" href="flow.php">购物车</a><span class="txt-cartcount" style="display: none;">0</span><div class="shopcart-list"><div class="empty">您的购物车内暂时没有商品，去<a href="index.php">首页</a>挑选</div></div>';
	}
    return $str;
}
 ...
 * @return  string
 */
function insert_cart_flow_content()
{
    ...
    $output = $GLOBALS['smarty']->fetch('library/cart_flow_content.lbi');
    return $output;
}
    ...
    foreach ($res AS $row)
    {
        ...
        switch ($row['media_type'])
        {
            case 0: // 图片广告
                ...
                target='_blank'><img src='$src' border='0' width='$row[ad_width]'  height='$row[ad_height]'/></a>";
                break;
            case 1: // Flash
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
                        DATA_DIR . "/afficheimg/$row[ad_code]" : $row['ad_code'];
