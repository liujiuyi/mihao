...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    $ur_here    = '<a href=".">' . $GLOBALS['_LANG']['home'] . '<i></i></a>';
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $val)
                {
                    ...
                    $ur_here   .= ' <a href="' . build_uri($type, $args, $val['cat_name']) . '">'. ' > ' .
                                    htmlspecialchars($val['cat_name']) . '<i></i></a>' ;
                }
            }
        }
        /* 处理无分类的 */
        else
        {
            ...
            if ('group_buy' == $filename)
            {
                ...
                $ur_here   .= ' <a href="group_buy.php">' .
                                $GLOBALS['_LANG']['group_buy_goods'] . '<i></i></a>';
            }
            /* 拍卖 */
            ...
            elseif ('snatch' == $filename)
            {
                ...
                $args       = array('id' => '0');
                $ur_here   .= ' <a href="snatch.php">' .                                 $GLOBALS['_LANG']['snatch_list'] . '<i></i></a>';
            }
            /* 批发 */
            ...
            elseif ('exchange' == $filename)
            {
                ...
                $ur_here   .= ' <a href="exchange.php">' .
                                $GLOBALS['_LANG']['exchange'] . '<i></i></a>';
            }
			  /* 限时抢购 */
            elseif ('group_buy' == $filename)
            {
                $page_title = '限时抢购_' . $page_title;
                $ur_here   .= ' <a href="flash_sale.php">限时抢购<i></i></a>';
            }
			 /* 晒单 */
    ...
    if (!empty($str))
    {
        ...
        $ur_here    .= '<a href="javascript:void(0);">'.$str.'</a>';
    }
...
function show_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
	...
	$GLOBALS['smarty']->assign('script_name', 'message');
...
function assign_comment($id, $type, $page = 1)
{
    ...
    $sql = 'SELECT c.*,u.rank_points FROM ' . $GLOBALS['ecs']->table('comment').' AS c LEFT JOIN '.$GLOBALS['ecs']->table('users')." AS u ON c.user_id=u.user_id WHERE id_value = '$id' AND comment_type = '$type' AND c.status = 1 AND c.parent_id = 0".
	...
	/*获取会员等级数据*/
	$sql  = 'SELECT min_points,max_points FROM '.$GLOBALS['ecs']->table('user_rank').' WHERE 1 ORDER BY rank_id ASC';
	$rank_res = $GLOBALS['db']->GetAll($sql);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		if(empty($row['rank_points']) || $row['user_id'] == 0)
		{
			 $arr[$row['comment_id']]['rank_portrait'] = 'images/rank_portrait_0.png'; 
		}
		else
		{
			foreach($rank_res as $key=>$value)
			{
				if($row['rank_points'] >= $value['min_points'] && $row['rank_points'] < $value['max_points'])
				{
					 $arr[$row['comment_id']]['rank_portrait'] = 'images/rank_portrait_'.$key.'.png';
				}
			}
		}
...
function get_top_article($cat_id)
{
 ...
 * @return  array
 */
function article_categories_tree($cat_id = 0 ,$id = 0)
{	
	$top_id = get_top_article($id);
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('article_cat') . " WHERE cat_id = '$cat_id'";
        $parent_id = $GLOBALS['db']->getOne($sql);
