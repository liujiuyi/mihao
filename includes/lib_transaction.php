 ...
 * @param   int         $num            列表最大数量
 * @param   int         $start          列表起始位置
 * @return  array       $order_list     晒单列表
 * 
 * @author guan 
 */

function get_user_orders_goods($user_id, $num = 10, $start = 0, $order_id = 0)
{
	/* 取得订单列表 */
	$arr    = array();
	...
	if(empty($order_id))
	{
		$sql = "SELECT distinct i.order_id, g.is_single, i.order_sn, i.order_status, i.shipping_status, i.pay_status, i.add_time, g.goods_id, g.goods_name, go.goods_thumb," .
				"(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee ".
				" FROM " .$GLOBALS['ecs']->table('order_info') . "AS i, " . $GLOBALS['ecs']->table('order_goods') . " AS g," .
				 $GLOBALS['ecs']->table('goods') . " AS go " .
				" WHERE ( i.user_id='$user_id' AND i.shipping_status=" . SS_RECEIVED . " ) AND i.order_id = g.order_id AND g.goods_id=go.goods_id ORDER BY i.add_time DESC";
	}
	else
	{
		$sql = "SELECT distinct i.order_id, g.is_single, i.order_sn, i.order_status, i.shipping_status, i.pay_status, i.add_time, g.goods_id, g.goods_name, go.goods_thumb," .
				...
				" FROM " .$GLOBALS['ecs']->table('order_info') . "AS i, " . $GLOBALS['ecs']->table('order_goods') . " AS g," .
				 $GLOBALS['ecs']->table('goods') . " AS go " .
				" WHERE ( i.user_id='$user_id' AND i.shipping_status=" . SS_RECEIVED . " AND i.order_id='$order_id') AND g.order_id = i.order_id AND g.goods_id=go.goods_id ORDER BY i.add_time DESC";
	}
	$res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
	...
	while ($row = $GLOBALS['db']->fetchRow($res))
	{
		if($row['shipping_status'] == SS_RECEIVED)
		{
			...
			//@@author guan start 查询是否评论 
			$sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('comment') . " WHERE user_id='$user_id' AND id_value='$row[goods_id]'";
			if($GLOBALS['db']->getOne($sql))
			{
				$row['comment'] = '<a style="color:#63C627;" href=javascript:void(0)>已评论</a>';
			}
			else
			{
				$row['comment'] = '<a style="color:blue;" href=goods.php?id=' . $row[goods_id] . '>发表评论</a>';
			}
			
			//@author guan 查询是否评论 end
			
			if($row['is_single'] == 0)
			{
				$row['single_sun'] = '<a style="color:blue;" href=user.php?act=single_sun&order_id=' .$row['order_id'] . '&goods_id=' . $row['goods_id'] . '>发表晒单</a>';
			}
			elseif($row['is_single'] == 1)
			{
				$row['single_sun'] = '<font style="color:red;">审核中</font>';
			}
			elseif($row['is_single'] == 2)
			{
				$sql = "SELECT single_id, integ FROM " . $GLOBALS['ecs']->table('single') . " WHERE goods_id = '$row[goods_id]' AND order_id=" . $row['order_id'];
				$single = $GLOBALS['db']->getRow($sql);
				$row['single_sun'] = '<a style="color:#ed1c24;" href=javascript:void(0)>已晒单</a>';
				
				//晒单赠送的积分数
				$row['integ'] = "<font style='color:red;'>（已获" . $single['integ'] . "积分）</font>";
			}
			else
			{
				$row['single_sun'] = '<a style="color:block;" href=user.php?act=single_sun&order_id=' .$row['order_id'] . '>审核未通过</a>';
			}
			...
			$arr[] = array('order_id'       => $row['order_id'],
					...
					'total_fee'      => price_format($row['total_fee'], false),
					'handler'        => $row['handler'],
					'single_sun'	 => $row['single_sun'],
					'is_single'	     => $row['is_single'],
					'goods_id'      => $row['goods_id'],
					'cat_id'      => $row['cat_id'],
					'comment'      => $row['comment'],
					'integ'      => $row['integ'],
					'single_order_time'   => $row['add_time']);
		}
	}
