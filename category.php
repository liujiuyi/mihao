...
$default_sort_order_type   = $_CFG['sort_order_type'] == '0' ? 'goods_id' : ($_CFG['sort_order_type'] == '1' ? 'goods_id' : 'last_update');
...
$display  = (isset($_REQUEST['display']) && in_array(trim(strtolower($_REQUEST['display'])), array('grid', 'list', 'text'))) ? trim($_REQUEST['display'])  : (isset($_COOKIE['ECS']['display']) ? $_COOKIE['ECS']['display'] : $default_display_type);
...
{
	//by guan start
	$hot_comment_ads = get_hot_comment_ads(221);
    $smarty->assign('hot_comment_ads',       $hot_comment_ads);
	
	
	$single_show = get_single_show();
    $smarty->assign('single_show',       $single_show);
	//by guan end
...
if (!$smarty->is_cached($dwt_name.'.dwt', $cache_id))
{
	...
	$smarty->assign('level_category',     get_level_category($cat_id));  // 获取同级分类
	...
	{
		$smarty->assign('categories_child',      get_cat_child($cat_id));
		$smarty->assign('categories_child_goods',      get_cat_child_goods($cat_id));
		//print_r(get_cat_child_goods($cat_id));
		$smarty->assign('like_goods',      like_goods($cat_id));
		$smarty->assign('browse_related',      browse_related());
	}
	else
	{
		$smarty->assign('sales_volume',    sales_volume($cat_id));
		$smarty->assign('categories_left',  get_categories_tree_left($cat_id)); 
		$smarty->assign('on_cat_name',  get_on_cat_name($cat_id));	
		$smarty->assign('crumb_list',  crumb_list($cat_id));
		$smarty->assign('browse_related',      browse_related());	
		$smarty->assign('like_goods',      like_goods($cat_id));
    ...
    $brand_list = array_merge($arr, get_brands2($cat_id));
	...
	//新增分类页商品相册 by mike start 20131128
