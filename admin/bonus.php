...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$ecs->table('bonus_type')." (type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,min_goods_amount,act_range,act_range_ext)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
            ...
            '$use_enddate',
            '$_POST[send_type]',
            '$min_amount','" . floatval($_POST['min_goods_amount']) . "','$act_range','$act_range_ext')";
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    /* 取得优惠范围 */
    $act_range_ext = array();
    if ($bonus_arr['act_range'] != FAR_ALL && !empty($bonus_arr['act_range_ext']))
    {
     if ($bonus_arr['act_range'] == FAR_CATEGORY)
     {
      $sql = "SELECT cat_id AS id, cat_name AS name FROM " . $ecs->table('category') .
      " WHERE cat_id " . db_create_in($bonus_arr['act_range_ext']);
     }
     elseif ($bonus_arr['act_range'] == FAR_BRAND)
     {
      $sql = "SELECT brand_id AS id, brand_name AS name FROM " . $ecs->table('brand') .
      " WHERE brand_id " . db_create_in($bonus_arr['act_range_ext']);
     }
     else
     {
      $sql = "SELECT goods_id AS id, goods_name AS name FROM " . $ecs->table('goods') .
      " WHERE goods_id " . db_create_in($bonus_arr['act_range_ext']);
     }
