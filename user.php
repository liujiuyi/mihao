...
/* 显示页面的action列表 */
$ui_arr = array('register', 'check_comm', 'login', 'profile', 'order_list', 'single_sun', 'single_sun_insert', 'single_list', 'order_detail', 'address_list', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer');
 $smarty->assign('categories',       get_categories_tree_pro()); // 分类树
/* 未登录处理 */
...
//@guan single sun start
/* 查看晒单页面 */
elseif ($action == 'single_sun')
{
// 	get_cat_articles()
	include_once(ROOT_PATH . 'includes/lib_transaction.php');
	...
	$sql = "SELECT order_id FROM " . $ecs->table('single') . " WHERE goods_id=" . $gid;
	if($db->getRow($sql))
	{
	...
	foreach($orders as $key => $val)
	{
		if($val['is_single'] == 0)
		{
			$arr[] = array('order_id'       => $val['order_id'],
					...
					'order_time'     => local_date($GLOBALS['_CFG']['time_format'], $val['add_time']),
					'order_status'   => $val['order_status'],
					'total_fee'      => price_format($val['total_fee'], false),
					'handler'        => $val['handler'],
					'single_sun'	 => $val['single_sun'],
					'is_single'	     => $val['is_single'],
					'goods_id'      => $val['goods_id'],
					'cat_id'      => $val['cat_id'],
					'single_order_time'   => $val['add_time']);
		}
	}
...
elseif ($action == 'single_sun_insert')
{
	...
	if(empty($_POST['desc_sn']))
	{
		show_message('请填写晒单描述', $_LANG['back_page_up'], "user.php?act=single_sun&order_id=$order_id", 'error');
	}

	$id = !empty($_POST['order_id']) ? $_POST['order_id'] : 0;
	$go_id_arr = explode('|', $id);
	
	if(empty($go_id_arr[0]) || empty($go_id_arr[1]))
	{
		ecs_header("Location: user.php?act=order_list\n");
		exit;
	}
	$sql = "SELECT * FROM " . $ecs->table('single') . " WHERE order_id='$go_id_arr[0]' AND goods_id = $go_id_arr[1]";
	if(($db->getOne($sql)))
	{
		show_message('-_-这个订单商品已经晒单了，重新选择一个吧', $_LANG['back_page_up'], "user.php?act=single_sun&order_id=$go_id_arr[0]&goods_id=$go_id_arr[1]", 'error');
	}
	...
	foreach ($_FILES['img_url']['error'] AS $key => $value)
	{
		if ($value == 0)
		{
			...
			{
				show_message($_LANG['invalid_img_url'], $_LANG['back_page_up'], "user.php?act=single_sun&order_id=$go_id_arr[0]&goods_id=$go_id_arr[1]", 'error');
			}
		}
		elseif ($value == 1)
		{
			show_message($_LANG['img_url_too_big'], $_LANG['back_page_up'], "user.php?act=single_sun&order_id=$go_id_arr[0]&goods_id=$go_id_arr[1]", 'error');
		}
	...
	$title_sn = !empty($_POST['title_sn']) ? $_POST['title_sn'] : '';
	$desc_sn = !empty($_POST['desc_sn']) ? $_POST['desc_sn'] : '';
	$user_name = $_SESSION['user_name'];
	$time = gmtime();
	$orders = get_user_orders_goods($user_id, 10, 0, $go_id_arr[0]);
	...
	'$go_id_arr[0]', '$title_sn', '$desc_sn', 0, '$user_name', 0, '$order_sn', '$time', '$go_id_arr[1]', '$goods_name', '$user_id', '$order_time', $cat_id)";
	$db->query($sql);
	/* 商品编号 */
	...
	if(!empty($single_id))
	{
		...
		$sql = "UPDATE " . $ecs->table('order_goods') . " SET is_single = '1' WHERE order_id='$go_id_arr[0]' AND goods_id='$go_id_arr[1]'";
		if($db->query($sql))
		{
		...
		{
			show_message($_LANG['single_error'], $_LANG['back_page_up'], "user.php?act=single_sun&order_id=$go_id_arr[0]&goods_id=$go_id_arr[1]", 'error');
		}
	}
	else
	{
...
/* 查看晒单列表 */
elseif ($action == 'single_list')
{
	include_once(ROOT_PATH . 'includes/lib_transaction.php');

	$page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;

	$order_id_arr = $db->getAll("SELECT order_id FROM " . $ecs->table('order_info') . "WHERE user_id = '$user_id' AND shipping_status = " . SS_RECEIVED);
	if(!empty($order_id_arr))
	{
		...
		$record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_goods'). " WHERE order_id IN($order_id)");
...
elseif($action == 'check_comm')
{
	...
	if($content == "")
	{
		show_message($_LANG['comm_content'], $_LANG['back_page_up'], "single_sun.php?act=single_show&sid=$sid", 'error');
	}
	...
	if(empty($is_ajax))
	{
		...
		{
			show_message($_LANG['comm_is_user'], $_LANG['back_page_up'], "single_sun.php?act=single_show&sid=$sid", 'error');
		}
	}
	...
	VALUES('101', '0', 'admin@admin.com', '$user_name', '$content', 5, '$addtime', '$ip', 0, 0, $user_id, '$sid')";
	$db->query($sql);
	$single_id = $db->insert_id();
	...
	if($single_id)
	{
		show_message($_LANG['comm_success'], $_LANG['back_page_up'], "single_sun.php?act=single_show&sid=$sid", 'success');
	}
	else
	{
		show_message($_LANG['comm_error'], $_LANG['back_page_up'], "single_sun.php?act=single_show&sid=$sid", 'error');
	}
}
