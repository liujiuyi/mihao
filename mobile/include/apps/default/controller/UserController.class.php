...
class UserController extends CommonController {
    ...
    public function __construct() {
        parent::__construct();
        // 属性赋值
        if(empty($_SESSION['user_id']) && !empty($_COOKIE['sopenid'])){
         $openid=$_COOKIE['sopenid'];
         $wch_users = init_users();
         $query_sql = "SELECT * FROM  ".$wch_users->table('users')." WHERE `openid` = '$openid'";
         $wch_M = M();
         $wch_arr = $wch_M->getRow($query_sql);
          
         if(!empty($wch_arr['user_name']))
         {
          $wch_user_name = $wch_arr['user_name'];
          ECTouch::user()->set_session($wch_user_name);
          ECTouch::user()->set_cookie($wch_user_name);
         }
        }
