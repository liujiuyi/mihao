...
class GoodsModel extends BaseModel {
    ...
    function get_goods_info($goods_id) {
        $time = gmtime();
        $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_name AS bonus_name, m.type_money AS bonus_money, ' .
    ...
    function get_goods_fittings($goods_list = array()) {
        ...
        $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, g.goods_weight AS goods_weight, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price " .
        ...
        foreach ($res as $key => $value) {
            ...
            /* 修正重量显示 */
            $arr[$temp_index]['goods_weight'] = (intval($value['goods_weight']) > 0) ? $value['goods_weight'] . L('kilogram') :($value['goods_weight'] * 1000) . L('gram');
            $temp_index++;
        }
        return $arr;
    }
