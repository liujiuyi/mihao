...
$smarty->assign('script_name', 'flow');
...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    else
    {
        ...
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent))
        {
			...
			$result['flow_content'] = insert_cart_flow_content();
...
elseif ($_REQUEST['step'] == 'add_to_cart_flow')
{
    include_once('includes/cls_json.php');
	$_POST['goods']=strip_tags(urldecode($_POST['goods']));
    $_POST['goods'] = json_str_iconv($_POST['goods']);
	...
	/*AJAX返回 by ecmoban*/
	//$result['show_info'] = insert_show_div_info($result['goods_number'],$result['goods_id'],$cart_goods['total']['goods_amount'],$cart_goods['total']['real_goods_count']);
...
elseif ($_REQUEST['step'] == 'login')
{
    ...
    else
    {
        ...
        if (!empty($_POST['act']) && $_POST['act'] == 'signin')
        {
            ...
            {
                $_SESSION['login_fail']++;
                show_message($_LANG['signin_failed'], '', 'flow.php?step=cart');
            }
        }
...
elseif ($_REQUEST['step']== 'ajax_update_cart')
{
     ...
     if (isset($_POST['rec_id']) && isset($_POST['goods_number']))
     {
		...
		$result['total_info'] = '<table class="bag_total_info_box">
									<tr>
										<th>商品件数：</th>
										<td>'.$result['total_goods_count'].' '.件.'</td>
									</tr>
									<tr>
										<th>商品总金额(不含运费)：</th>
									...
									<tr>
										<th>优惠：</th>
										<td>'.￥.''.$result['total_saving'].'</td>
									</tr>
									<tr>
										<th colspan="2">
											<div class="total-price">
												应付金额：
												<strong class="price" id="amount">'.￥.''.$result['total_price'].'</strong>
											</div>
										</th>
									</tr>
								</table>';
...
elseif ($_REQUEST['step'] == 'yesmywine_drop_goods')
{
	include_once('includes/cls_json.php');
	$json  = new JSON;
    $rec_id =json_str_iconv($_POST['rec_id']);
    flow_drop_cart_goods($rec_id);
	$result=insert_cart_info();
	die($json->encode($result));
}
...
function favourable_list($user_rank)
{
    ...
    while ($favourable = $GLOBALS['db']->fetchRow($res))
    {
        ...
        foreach ($favourable['gift'] as $key => $value)
        {
			...
			$sql = 'SELECT shop_price,goods_thumb FROM '.$GLOBALS['ecs']->table('goods')." WHERE goods_id=".$value['id'];
			$row = $GLOBALS['db']->getRow($sql);
			
			$favourable['gift'][$key]['shop_price']  = $row['shop_price'];
			$favourable['gift'][$key]['goods_thumb'] = $row['goods_thumb'];
 ...
 * 验证商品是否符合使用红包规则
 */
function check_bonus_used($bonus)
{
   $sql = "SELECT count(c.goods_id) " .
          "  FROM " . $GLOBALS['ecs']->table('cart') . " AS c, " . $GLOBALS['ecs']->table('goods') . " AS g " .
          " WHERE c.goods_id = g.goods_id " .
          "   AND c.session_id = '" . SESS_ID . "' " .
          "   AND c.rec_type = '" . CART_GENERAL_GOODS . "' " .
   ...
   /* 根据优惠范围修正sql */
   if ($bonus['act_range'] == FAR_ALL)
   {
      // sql do not change
   }
   elseif ($bonus['act_range'] == FAR_CATEGORY)
   {
     /* 取得优惠范围分类的所有下级分类 */
     $id_list = array();
     $cat_list = explode(',', $bonus['act_range_ext']);
     foreach ($cat_list as $id)
     {
       $id_list = array_merge($id_list, array_keys(cat_list(intval($id), 0, false)));
     }
   ...
   elseif ($bonus['act_range'] == FAR_BRAND)
   {
      $id_list = explode(',', $bonus['act_range_ext']);
